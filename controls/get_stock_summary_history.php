  <?php
    ...
    if(isset($_POST['stock_start_date']) && isset($_POST['stock_end_date'])){
        ...
        $query="SELECT item_id,job_id,datetime,stat,price, SUM(qty) FROM tbl_job_item WHERE DATE(datetime) BETWEEN '$stock_start_date' AND '$stock_end_date' GROUP BY item_id ORDER BY SUM(qty) DESC";
        // echo "SELECT item_id,job_id,datetime,stat,price, SUM(qty) FROM tbl_job_item WHERE DATE(datetime) BETWEEN '$stock_start_date' AND '$stock_end_date' GROUP BY item_id ORDER BY SUM(qty) DESC";
        ...
        while ($sq=$getStocQty->fetch_array()) {
          ...
          $dateJob=$sq[2];
          $priceBatchId=$sq[3];
          $itemSelling=(double)$sq[4];
          /////////////////
          $JobYear = date('Y', strtotime($dateJob));
          $JobNumber=10000+$jobId;
      ...
      $obj='
            <tr>
                ...
                <td>BAE/JOB/'.$JobYear.'/'.$JobNumber.'</td>
                <td>'.$itemName.'</td>
                <td>'.$itemNumber.'</td>
